...
public:
	...
	//默认当光线未与物体碰撞时返回的颜色为黑色，即只有光源打到的部分会亮。
	...
	ray get_ray(int i, int j)
	{
		...
		return ray(camera, dir, random_double());
	}
	...
	void render(objectlist objects,const char *fname)
	{
		initialize();
		std::ofstream file;
		file.open(fname);
...
private:
	...
	color ray_color(const ray& r, int depth, const objectlist& objects) const
	{
		...
		{
			color scattered_color, emitted_color;
			color attenuation;
			ray scattered;
			//光线打到非光源物体
			if (rec.mat->scatter(r, rec, attenuation, scattered))
				scattered_color = ray_color(scattered, depth - 1, objects) * attenuation;
				//return attenuation;
			else
				scattered_color = color(0, 0, 0);
			emitted_color = rec.mat->emitted(rec.u, rec.v, rec.p);
